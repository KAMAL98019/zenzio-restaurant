class Offer {
  ...
  bool isActive() {
    ...
    // An offer is active if its status is 'ACTIVE' and the current date is within or on the start and end dates.
    return status == 'ACTIVE' &&
        (now.isAfter(startDate) || now.isAtSameMomentAs(startDate)) &&
        (now.isBefore(endDate.add(const Duration(days: 1))) || now.isAtSameMomentAs(endDate.add(const Duration(days: 1))));
  }
  ...
  bool isUpcoming() {
    final now = DateTime.now();
    // An offer is upcoming if its start date is in the future.
    return now.isBefore(startDate);
  }

  bool isExpired() {
    final now = DateTime.now();
    // An offer is expired if the current date is strictly after its end date.
